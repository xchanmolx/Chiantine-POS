...
namespace PointOfSaleViewerForm
{
    public partial class DisplaySalesProductForm : Form
    {
        ...
        //ConfigurationManager.ConnectionStrings["POS"].ConnectionString

        DataTable dt = new DataTable("Orders");
        //SqlDataAdapter sda = new SqlDataAdapter();
        private void DisplaySalesProductForm_Load(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection cn = new SqlConnection(GlobalConfig.CnnString(db)))
                {
                    ...
                    //    cn.Open();
                    //using (SqlDataAdapter sda = new SqlDataAdapter("SELECT * FROM Orders", cn))
                    //{
                    //    sda.Fill(dt);
                    //    dgvSalesProduct.DataSource = dt;
        ...
        private void txtSearchSalesProduct_TextChanged(object sender, EventArgs e)
        {
            ...
            //dv.RowFilter = string.Format("Convert(Date, 'System.String') LIKE '%{0}%' ", txtSearchSalesProduct.Text);
            //dgvSalesProduct.DataSource = dv.ToTable();
        }
